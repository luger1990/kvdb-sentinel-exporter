...
class RedisMetricsCollector:
    ...
    def __init__(self):
        ...
        # 基本指标
        self.up = Gauge('kvdb_up', 'Redis实例是否在线', ['db_instance', 'group_name', 'role', 'sentinel_name'], registry=self.registry)
        ...
        self.commands_total = Gauge('kvdb_commands_total', 'Redis各命令的执行次数',
                                    ['db_instance', 'group_name', 'role', 'command', 'sentinel_name'], registry=self.registry)
    ...
    def collect_metrics(self, redis_info_dict, sentinel_name):
        ...
        for instance, info in redis_info_dict.items():
            ...
            self.node_role.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(is_master)

            # 设置引擎类型 (使用新的type字段)
            self.engine_type.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(node_type)

            # 实例运行时间
            uptime = info.get('uptime_in_seconds', 0)
            self.uptime_in_seconds.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(uptime)

            # 客户端连接
            if 'connected_clients' in info:
                self.connected_clients.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['connected_clients'])

            # 最大客户端连接
            if 'maxclients' in info:
                self.max_clients.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(info['maxclients'])

            # 阻塞客户端
            if 'blocked_clients' in info:
                self.blocked_clients.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['blocked_clients'])

            # 内存使用情况
            if 'used_memory_rss' in info:
                self.memory_used_bytes.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
            ...
            self.memory_max_bytes.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(max_memory)

            # 处理命令总数
            if 'total_commands_processed' in info:
                self.commands_processed_total.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['total_commands_processed'])

            # 网络流量
            if 'total_net_input_bytes' in info:
                self.net_input_bytes_total.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['total_net_input_bytes'])

            if 'total_net_output_bytes' in info:
                self.net_output_bytes_total.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['total_net_output_bytes'])

            # 网络流量速率
            if 'instantaneous_input_kbps' in info:
                self.net_input_kbps.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['instantaneous_input_kbps'])

            if 'instantaneous_output_kbps' in info:
                self.net_output_kbps.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
            ...
            for key, value in info.items():
                if key.startswith('db'):
                    ...
                    if isinstance(value, dict):
                        if 'keys' in value:
                            self.db_keys.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name, db=db_name).set(
                                value['keys'])
                        if 'expires' in value:
                            self.db_keys_expiring.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name,
                                                         db=db_name).set(value['expires'])

            # 驱逐的键数量
            if 'evicted_keys' in info:
                self.evicted_keys_total.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['evicted_keys'])
            ...
            try:
                ...
                if 'slowlog_len' in info:
                    self.slowlog_length.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                        info['slowlog_len'])
            ...
            if 'connected_slaves' in info:
                self.connected_slaves.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['connected_slaves'])

            # 主节点与从节点最后通信时间
            if 'master_last_io_seconds_ago' in info:
                self.master_last_io_seconds_ago.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['master_last_io_seconds_ago'])

            # 复制偏移量
            if 'master_repl_offset' in info:
                self.master_repl_offset.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
                    info['master_repl_offset'])

            # 每秒执行的操作数
            if 'instantaneous_ops_per_sec' in info:
                self.instantaneous_ops_per_sec.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(
            ...
            self.version.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(version_value)

            # 端口信息
            if 'tcp_port' in info:
                self.port.labels(db_instance=instance, group_name=master_name, role=role, sentinel_name=sentinel_name).set(info['tcp_port'])

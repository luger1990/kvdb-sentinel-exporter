...
class RedisMetricsCollector:
    ...
    def __init__(self):
        ...
        # 基本指标
        self.up = Gauge('kvdb_up', 'Redis实例是否在线', ['instance', 'group_name', 'role'], registry=self.registry)

        # 节点角色指标，1表示主库，0表示从库
        self.node_role = Gauge('kvdb_role', 'Redis节点角色(1=主库, 0=从库)',
                              ['instance', 'group_name'], registry=self.registry)
        
        # 按照用户要求定义的指标
        self.uptime_in_seconds = Gauge('kvdb_uptime_in_seconds', 'Redis实例运行时间（秒）',
                                       ['instance', 'group_name', 'role'], registry=self.registry)
        ...
        self.db_keys = Gauge('kvdb_db_keys', 'Redis数据库中的键数量', 
                           ['instance', 'group_name', 'role', 'db'], registry=self.registry)
        ...
        self.commands_total = Gauge('kvdb_commands_total', 'Redis各命令的执行次数',
                                    ['instance', 'group_name', 'role', 'command'], registry=self.registry)
    ...
    def collect_metrics(self, redis_info_dict):
        """从Redis信息收集指标"""
        ...
        for instance, info in redis_info_dict.items():
            ...
            # 设置实例在线状态
            self.up.labels(instance=instance, group_name=master_name, role=role).set(1)
            
            # 设置节点角色
            is_master = 1 if role == 'master' else 0
            self.node_role.labels(instance=instance, group_name=master_name).set(is_master)

            # 设置引擎类型 (使用新的type字段)
            self.engine_type.labels(instance=instance, group_name=master_name, role=role).set(node_type)

            # 实例运行时间
            uptime = info.get('uptime_in_seconds', 0)
            self.uptime_in_seconds.labels(instance=instance, group_name=master_name, role=role).set(uptime)

            # 客户端连接
            if 'connected_clients' in info:
                self.connected_clients.labels(instance=instance, group_name=master_name, role=role).set(
                    info['connected_clients'])

            # 最大客户端连接
            if 'maxclients' in info:
                self.max_clients.labels(instance=instance, group_name=master_name, role=role).set(info['maxclients'])

            # 阻塞客户端
            if 'blocked_clients' in info:
                self.blocked_clients.labels(instance=instance, group_name=master_name, role=role).set(
                    info['blocked_clients'])

            # 内存使用情况
            if 'used_memory_rss' in info:
                self.memory_used_bytes.labels(instance=instance, group_name=master_name, role=role).set(
            ...
            self.memory_max_bytes.labels(instance=instance, group_name=master_name, role=role).set(max_memory)

            # 处理命令总数
            if 'total_commands_processed' in info:
                self.commands_processed_total.labels(instance=instance, group_name=master_name, role=role).set(
                    info['total_commands_processed'])

            # 网络流量
            if 'total_net_input_bytes' in info:
                self.net_input_bytes_total.labels(instance=instance, group_name=master_name, role=role).set(
                    info['total_net_input_bytes'])

            if 'total_net_output_bytes' in info:
                self.net_output_bytes_total.labels(instance=instance, group_name=master_name, role=role).set(
                    info['total_net_output_bytes'])

            # 网络流量速率
            if 'instantaneous_input_kbps' in info:
                self.net_input_kbps.labels(instance=instance, group_name=master_name, role=role).set(
                    info['instantaneous_input_kbps'])

            if 'instantaneous_output_kbps' in info:
                self.net_output_kbps.labels(instance=instance, group_name=master_name, role=role).set(
            ...
            for key, value in info.items():
                if key.startswith('db'):
                    ...
                    if isinstance(value, dict):
                        if 'keys' in value:
                            self.db_keys.labels(instance=instance, group_name=master_name, role=role, db=db_name).set(
                                value['keys'])
                        if 'expires' in value:
                            self.db_keys_expiring.labels(instance=instance, group_name=master_name, role=role,
                                                         db=db_name).set(value['expires'])

            # 驱逐的键数量
            if 'evicted_keys' in info:
                self.evicted_keys_total.labels(instance=instance, group_name=master_name, role=role).set(
                    info['evicted_keys'])
            ...
            try:
                ...
                if 'slowlog_len' in info:
                    self.slowlog_length.labels(instance=instance, group_name=master_name, role=role).set(
                        info['slowlog_len'])
            ...
            if 'connected_slaves' in info:
                self.connected_slaves.labels(instance=instance, group_name=master_name, role=role).set(
                    info['connected_slaves'])

            # 主节点与从节点最后通信时间
            if 'master_last_io_seconds_ago' in info:
                self.master_last_io_seconds_ago.labels(instance=instance, group_name=master_name, role=role).set(
                    info['master_last_io_seconds_ago'])

            # 复制偏移量
            if 'master_repl_offset' in info:
                self.master_repl_offset.labels(instance=instance, group_name=master_name, role=role).set(
                    info['master_repl_offset'])

            # 每秒执行的操作数
            if 'instantaneous_ops_per_sec' in info:
                self.instantaneous_ops_per_sec.labels(instance=instance, group_name=master_name, role=role).set(
            ...
            self.version.labels(instance=instance, group_name=master_name, role=role).set(version_value)

            # 端口信息
            if 'tcp_port' in info:
                self.port.labels(instance=instance, group_name=master_name, role=role).set(info['tcp_port'])
